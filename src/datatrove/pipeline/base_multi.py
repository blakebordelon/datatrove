...
class BaseReader_Multi(PipelineStep):
    ...
    def _default_adapter(self, data: dict, path: str, id_in_file: int | str):
        ...
        return {
            "text": "".join([data.pop(key, "") for key in self.text_keys]),
        ...
        if not parsed_data.get("text", None):
            if not self._empty_warning:
                ...
                logger.warning(
                    ...
                    f'Is your `text_key` ("{self.text_keys}") correct? Available keys: {list(data.keys())}'
                )
            return None
...
class BaseDiskReader_Multi(BaseReader_Multi):
    ...
    def __init__(
        ...
        super().__init__(limit, skip, adapter, text_keys, id_key, default_metadata)
...
class JsonlReader_Multi(BaseDiskReader_Multi):
